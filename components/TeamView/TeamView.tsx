...
import { getParticipantByEmail, getTeamById, removeParticipantFromTeam } from "@/functions/db";
...
export async function TeamView({ participantEmail }: { participantEmail: string }) {
  ...
  return (
    <>
      ...
      <h2>your team</h2>
      <ul>
        {participants.map((participant) => <li key={participant.name}>{participant.name.toLowerCase()}</li>)}
      </ul>
